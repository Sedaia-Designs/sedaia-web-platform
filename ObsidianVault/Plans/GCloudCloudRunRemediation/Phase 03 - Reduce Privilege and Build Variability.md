...
- [ ] Record before-and-after IAM policies, test build IDs, denied permissions encountered, final rationale, and a rollback procedure for IAM changes.[^platform-identities]
...
- [ ] Generate and retain an SBOM and vulnerability-scan result for the released image, or record an explicit risk acceptance and owner if container scanning remains disabled.[^platform-supply-chain]
...
[^platform-identities]: [[../SedaiaPlatformBuildout/Phase 10 - CI Environments Secrets and Supply Chain#Ordered steps|Overall Platform Buildout Phase 10]] requires dedicated least-privilege environment identities, serialized protected mutations, exact provenance, and permission-negative evidence across all components; this phase supplies the API/Cloud Run portion.
[^platform-supply-chain]: [[../SedaiaPlatformBuildout/Phase 10 - CI Environments Secrets and Supply Chain#Ordered steps|Overall Platform Buildout Phase 10]] also requires pinned build inputs, deterministic container digest promotion, retained SBOM/provenance, vulnerability/license results, and expiring exceptions.
