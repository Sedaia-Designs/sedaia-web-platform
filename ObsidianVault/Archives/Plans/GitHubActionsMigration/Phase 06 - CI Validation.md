...
**Status: Repository implementation complete; hosted run evidence pending.**
The workflow preserves the stable `Ktor API`, `Frontends`, and `API contract`
job names and contains no production environment, OIDC, or deployment step.
Pull-request and merged-`main` run URLs are not recorded.

Exercise the API, frontend, and API-contract jobs on a pull request and on
`main`. Confirm expected check names, failure reporting, and
